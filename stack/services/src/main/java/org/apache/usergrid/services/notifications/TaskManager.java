...
public class TaskManager {
    ...
    public void completed(Notifier notifier, Receipt receipt, UUID deviceUUID, String newProviderId) throws Exception {
        if(logger.isDebugEnabled()) logger.debug("REMOVED {}", deviceUUID);
        try {
            if(logger.isDebugEnabled()) logger.debug("notification {} removing device {} from remaining", notification.getUuid(), deviceUUID);

            EntityRef deviceRef = new SimpleEntityRef(Device.ENTITY_TYPE, deviceUUID);
            if (receipt != null) {
                if(logger.isDebugEnabled()) logger.debug("notification {} sent to device {}. saving receipt.", notification.getUuid(), deviceUUID);
                receipt.setSent(System.currentTimeMillis());
                this.saveReceipt(notification, deviceRef, receipt,false);
                if(logger.isDebugEnabled()) logger.debug("notification {} receipt saved for device {}", notification.getUuid(), deviceUUID);
                successes.incrementAndGet();
            }

            if (newProviderId != null) {
                if(logger.isDebugEnabled()) logger.debug("notification {} replacing device {} notifierId", notification.getUuid(), deviceUUID);
                replaceProviderId(deviceRef, notifier, newProviderId);
            }

            if(logger.isDebugEnabled()) logger.debug("notification {} completed device {}", notification.getUuid(), deviceUUID);

        } finally {
            if(logger.isDebugEnabled()) logger.debug("COUNT is: {}", successes.get());

...
//                .payload( data ).calculateMD5().contentType( mimeType );
    ...
    class UploadWorker implements Callable<Void> {
        ...
        public Void call() {
            ...
            if(logger.isDebugEnabled()) logger.debug("Writing temp file for S3 upload");
            ...
            try {
                ...
                if(logger.isDebugEnabled()) logger.debug("Write temp file {} length {}", tempFile.getName(), written);
            ...
            if ( tempFile.length() > maxSizeBytes ) {
                if(logger.isDebugEnabled()) logger.debug("File too large. Temp file size (bytes) = {}, " +
            ...
            try {  // start the upload
                ...
                if(logger.isDebugEnabled()) logger.debug( "S3 upload thread started" );
...
//                    .payload( tempFile ).calculateMD5().contentType( mimeType );
                ...
                if(logger.isDebugEnabled()) logger.debug( "S3 upload starting" );

                String eTag = blobStore.putBlob( bucketName, blob );

                if(logger.isDebugEnabled()) logger.debug( "S3 upload complete eTag=" + eTag);

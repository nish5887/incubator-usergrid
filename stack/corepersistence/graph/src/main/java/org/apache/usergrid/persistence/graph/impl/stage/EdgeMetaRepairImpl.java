...
public class EdgeMetaRepairImpl implements EdgeMetaRepair {
    ...
    private Observable<Integer> clearTypes( final ApplicationScope scope, final Id node, final String edgeType,
        ...
        Observable<Integer> deleteCounts = serialization.loadEdgeSubTypes( scope, node, edgeType, maxTimestamp ).buffer(
                ...
                .flatMap( types -> {
                    ...
                    for ( final String subType : types ) {
                        ...
                        logger.debug("Checking for edges with nodeId {}, type {}, and subtype {}", node, edgeType,
                            subType);
                        ...
                        Observable<Integer> search =
                                ...
                                serialization.loadEdges( scope, node, edgeType, subType, maxTimestamp )
                                             ...
                                             .doOnNext( count -> {
                                                  ...
                                                  **/
                                                 if ( count != 0 ) {
                                                     if(logger.isDebugEnabled()) logger.debug("Found edge with nodeId {}, type {}, "
                                                 ...
                                                 if(logger.isDebugEnabled()) logger.debug("No edges with nodeId {}, type {}, "
                    ...
                    return MathObservable.sumInteger( Observable.merge( checks ) )
                                         .doOnNext( count -> {
                                                 ...
                                                 if(logger.isDebugEnabled()) logger.debug("Executing batch for subtype deletion with " +
        ...
        return MathObservable.sumInteger( deleteCounts ).lastOrDefault( 0 ).doOnNext( subTypeUsedCount -> {
            ...
            if ( subTypeUsedCount != 0 ) {
                if(logger.isDebugEnabled()) logger.debug("Type {} has {} subtypes in use as of maxTimestamp {}.  Not deleting type.", edgeType,
                    subTypeUsedCount, maxTimestamp);
                return;
            }


            if(logger.isDebugEnabled()) logger.debug("Type {} has no subtypes in use as of maxTimestamp {}.  Deleting type.", edgeType,
                maxTimestamp);
            try {
    ...
    private static class Log implements Action1<MarkedEdge> {
        ...
        public void call( final MarkedEdge markedEdge ) {
            if(logger.isDebugEnabled()) logger.debug("Emitting edge {}", markedEdge);
        }
    }
}
